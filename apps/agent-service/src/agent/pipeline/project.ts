...
export function detectTestCommand(files: Record<string, string>): string | null {
  const pkgPaths = Object.keys(files)
    .filter((p) => p === 'package.json' || p.endsWith('/package.json'))
    .sort((a, b) =>
      a === 'package.json' ? -1 : b === 'package.json' ? 1 : a.length - b.length,
    );
...
export function summarizeProject(files: Record<string, string>): string {
  ...
  for (const p of pkgPaths.sort((a, b) => a.length - b.length)) {
    ...
    if (Object.keys(deps).length > 0) {
      lines.push(`  dependencies (${Object.keys(deps).length}): ${Object.keys(deps).slice(0, 12).join(', ')}${Object.keys(deps).length > 12 ? ', …' : ''}`);
    }
...
export function detectVerificationCommands(files: Record<string, string>): string[] {
